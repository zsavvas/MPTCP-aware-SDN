...
public class TopologyManager implements IFloodlightModule, ITopologyService, IRoutingService, ILinkDiscoveryListener, IOFMessageListener {
    ...
    //global variables for the DFS search
...
public List<NodePortTuple> stripPath(List<NodePortTuple> path){
	List<NodePortTuple> strippedPath = new ArrayList<NodePortTuple>(path);

	strippedPath.remove(0);
	strippedPath.remove(0);
	strippedPath.remove(strippedPath.size()-1);
	strippedPath.remove(strippedPath.size()-1);
	return strippedPath;
}

	public synchronized ArrayList<Route> getECMPRoutes(DatapathId srcDpid, DatapathId dstDpid,
			boolean tunnelEnabled, int k){
		 ArrayList<Route> allRoutes = new ArrayList<Route>();
		  ...
		  ArrayList<Route> ecmpRoutes = new ArrayList<Route>();
		  ArrayList<Route> minRoutes = new ArrayList<Route>();
		  ...
		  if(r.getPath().size()==minLength)
			  ...
			  break;
                  ...
                  Collections.shuffle(minRoutes);
		  //Random randomGenerator= new Random();
		  //int index = randomGenerator.nextInt(16);
		  List<NodePortTuple> used = new ArrayList<NodePortTuple>();
		  //ecmpRoutes.add(allRoutes.get(index));
		  //used.addAll(stripPath(minRoutes.get(index).getPath()));
		  //used.addAll(allRoutes.get(index).getPath()); 
		 if(k>0){
		  for( Route r : minRoutes){  
			  if(intersection(r.getPath(),used)==0){
			  ecmpRoutes.add(r);
			  //used.addAll(stripPath(r.getPath()));
			  used.addAll(r.getPath());
			  }
			  if(ecmpRoutes.size()>=k){
					 break;
				}
		  }
		  }else{
	...
	@Override
	public synchronized ArrayList<Route> getRoutes(DatapathId srcDpid, DatapathId dstDpid,
			boolean tunnelEnabled) {
